...
class will{
  public $name;
  const Age = 12;
  public static $will; #只能通过 :: 进行调用(静态成员访问符)
...
读取不可访问方法的值时，__call() 会被调用
读取不可访问静态方法的值时， __callStatic() 会被调用
````
关于回调函数和闭包函数(匿名函数)
````
                                            关于回调函数
...
回调函数就是在给一个函数传入一个简单的参数无法解决问题的时候给他传入一个过程,从而达到目的
    ...
    1.1实例:
        ...
        1.1.2  call_user_func_array               调用回调函数，并把一个数组参数作为回调函数的参数,进行多次回调函数的复制调用,直至数组中的值都被执行完成
           ...
           // foo::bar got three and four
            ...
            call_user_func_array('index',array(1,2));
            //D:\phpstudy\phpstudy_pro\WWW\test.php:8:
            //array (size=2)
            //  0 => int 1
            //  1 => int 2
        1.1.3完整实例:
...
其他补充
````
//call_user_func(function ($name,$bb){
//    echo $name;
//    echo $bb;
//}, ...array(1, 2,3));
...
//    var_dump($name);
//}
//call_user_func_array('index',array(1,2,3));
//D:\phpstudy\phpstudy_pro\WWW\test.php:8:
//array (size=2)
//  0 => int 1
//  1 => int 2
